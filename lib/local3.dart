...
class local3 extends StatefulWidget {
 final String? location;
  final String? email;
  final int? id_u;

  const local3({Key? key, this.location, this.email, this.id_u})
  : super(key: key);

  @override
  State<local3> createState() => _loginState();
}

class _loginState extends State<local3> {
    ...
    if (response.statusCode == 200) {
      ...
      if (data is Map<String, dynamic>) {
        setState(() {
          ...
          tv_h.text = data['tv_h']?.toString() ?? '';
          tv_m.text = data['tv_m']?.toString() ?? '';

          tvlcd.text = data['tvlcd']?.toString() ?? '';
          tvlcd_h.text = data['tvlcd_h']?.toString() ?? '';
          tvlcd_m.text = data['tvlcd_m']?.toString() ?? '';

          tvled.text = data['tvled']?.toString() ?? '';
          tvled_h.text = data['tvled_h']?.toString() ?? '';
          tvled_m.text = data['tvled_m']?.toString() ?? '';

          com.text = data['com']?.toString() ?? '';
          com_h.text = data['com_h']?.toString() ?? '';
          com_m.text = data['com_m']?.toString() ?? '';

          lab.text = data['lab']?.toString() ?? '';
          lab_h.text = data['lab_h']?.toString() ?? '';
          lab_m.text = data['lab_m']?.toString() ?? '';

          // ตั้งค่าตัวแปร selectedBulbs ตามข้อมูลที่ดึงมา
          selectedBulbs1 = int.tryParse(data['tv'] ?? '') ?? 0;
          selectedBulbs2 = int.tryParse(data['tvlcd'] ?? '') ?? 0;
          selectedBulbs3 = int.tryParse(data['tvled'] ?? '') ?? 0;
          selectedBulbs4 = int.tryParse(data['com'] ?? '') ??
              0; // สมมติว่าใช้ตัวแปรนี้สำหรับ air_b
          selectedBulbs5 = int.tryParse(data['lab'] ?? '') ?? 0;
  ...
  Future update() async {
    String url = "http://172.20.10.2/flutter_login/local3up.php";
